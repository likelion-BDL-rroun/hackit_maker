...
export const TopBar = ({ onExport }: TopBarProps) => {
  const { title, setTitle, undo, redo, history } = useEditorStore();
  ...
  return (
    <div
      className="border-b border-gray-200 bg-white flex items-center justify-between px-4 lg:px-6 shadow-sm z-[100] relative shrink-0"
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={onExport}
          className="flex items-center gap-1.5 px-4 py-2 text-xs text-white bg-[#FF6000] hover:bg-[#E55600] rounded-[8px] shadow-sm hover:shadow-md transition-all active:scale-[0.97]"
